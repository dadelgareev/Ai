...
class LamodaScraper:
    ...
    def get_all_atrib_from_page(self, url):
        ...
        product_name_tag = soup.find('div', class_='x-premium-product-title__model-name')
        ...
        brand = other_atr_dict.get('product', {}).get('brand', {}).get('title', None)
        # Возвращаем изображения, атрибуты и категории
        return {
            "image_urls": image_urls,
            "attributes": attributes,
            "categories": categories_value,
            "title": product_name_tag,
            "price": price,
            "brand": brand
        }
    ...
    def create_and_append_csv_json(self, json_file, output_csv, main_category, grpc_client=None):
        ...
        # Поля CSV-файла
        fieldnames = ['Source', 'Source_csv', 'Guid', 'Image_url','Title', 'Main_photo', 'Guid_list', 'Article', 'Gender',
        ...
        with open(output_csv, 'a', newline='', encoding='utf-8') as old_csvfile, \
            ...
            for link_entry in unprocessed_links:
                ...
                subcategory = list(categories.keys())[0] if categories else 'Не найдено'
