...
class LamodaScraper:
    def __init__(self):
        ...
        "Верхняя одежда": [
            ...
            "Пальто меховые","Накидки и пончо", "Дублёнки", "Тренчкоты", "Кейпы и накидки"
        ],
        ...
        "Домашняя одежда": [
            ...
            "Пижамы с шортами","Трикотажные халаты","Флисовые халаты", "Сорочки"
        ...
        "Нижнее белье": [
            ...
            "Лифы-бандо", "Лифы-халтер","Бандажи", "Комплекты белья"
        ],
        ...
        "Пиджаки и костюмы": [
            ...
            "Костюмы и комбинезоны","Костюмы и жакеты","Вязаные костюмы", "Смокинги"
        ...
        "Прочее": ["Уход за одеждой","Шнурки","Уход за обувью","Экипировка","Спортивный инвентарь", "Аксессуары для путешествий",
                   "Аксессуары для обуви", "Пляжные принадлежности"],
        "Балетки": [
            "Балетки с квадратным носом", "Балетки с круглым носом", "Балетки с острым носом", "Балетки"
        ],
        "Ботильоны": [
    ...
    def get_all_atrib_from_page(self, url):
        ...
        description = other_atr_dict.get('product', {}).get('description', 'Описание не найдено')
        # Возвращаем изображения, атрибуты и категории
        return {
            "image_urls": image_urls,
            "attributes": attributes,
            "categories": categories_value,
            "all_categories": categories_all[1:], #скип категорию "Главная"
            "price": price,
            "brand": brand,
    ...
    def create_and_append_csv_json_fine_tuning(self, json_file, output_csv, main_category, grpc_client=None):
        ...
        # Определяем директорию для изображений
        if main_category[1] == "Man":
            images_dir = os.path.join("Lamoda_Photos", "Man", os.path.splitext(output_csv)[0])
        elif main_category[1] == "Woman":
            images_dir = os.path.join("Lamoda_Photos", "Woman", os.path.splitext(output_csv)[0])
        else:
            images_dir = os.path.splitext(output_csv)[0]  # Для других категорий
...
if __name__ == "__main__":
    ...
    html = scraper.fetch_page("https://www.lamoda.ru/p/rtlado792101/shoes-adidasoriginals-krossovki/")
    soup = BeautifulSoup(html, 'html.parser')
    dict = scraper.extract_payload(soup)
    print(dict)
    print(dict["product"])
    print(dict["product"].keys())
    print(dict["product"]["description"])
    print(dict["product"]["main_attributes"])
    print(scraper.get_all_atrib_from_page("https://www.lamoda.ru/p/rtlado792101/shoes-adidasoriginals-krossovki/"))
    href = ["https://www.lamoda.ru/p/rtlado792101/shoes-adidasoriginals-krossovki/"]
    scraper.update_links_file_json("test.json",href)
    scraper.create_and_append_csv_json_fine_tuning("test.json", "test.csv",scraper.list_categories["man_shoes"])
"""
